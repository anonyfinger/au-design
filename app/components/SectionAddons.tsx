...
const ADDON_ITEMS = [
  {
    icon: "📅",
    title: "출근부 정기 업데이트",
    desc: "매주·매월 출근부 디자인을 새로 제작해 드립니다. 한 번 제작 후에도 꾸준히 **신선한 이미지**를 유지할 수 있습니다.",
    tag: "월정기",
  },
  {
    icon: "🎉",
    title: "시즌·이벤트 배너 리뉴얼",
    desc: "명절·이벤트·프로모션 시즌에 맞춘 배너 재제작. **플랫폼 노출 우선권**을 유지하고 신규 고객 유입을 늘립니다.",
    tag: "시즌별",
  },
  {
    icon: "📰",
  ...
  },
  {
    icon: "🖼️",
    title: "구글 이미지 노출 관리",
    desc: "구글 이미지 검색에 업체 배너가 **지속 노출**되도록 관리합니다. 온라인 **브랜드 인지도**를 꾸준히 높입니다.",
    tag: "지속관리",
  },
  {
    icon: "🔄",
  ...
  },
  {
    icon: "➕",
    title: "신규 플랫폼 추가 등록",
    desc: "새로운 플랫폼에 등록할 때 기존 디자인을 해당 규격으로 변환·추가 제작해 드립니다. **멀티 플랫폼 운영**이 편해집니다.",
    tag: "수시",
  },
];
        ...
        <div className="addonsGrid">
          {ADDON_ITEMS.map((item) => (
            <article key={item.title} className="addonCard">
              ...
              <p className="addonDesc">{hl(item.desc)}</p>
            </article>
          ))}
        </div>
