...
export const metadata = {
  title: "배너·프로필 제작 가이드",
  description:
    "오피가이드 배너제작·오피가이드 배너 제작 가이드. 오피스타·오피뷰·외로운밤·오피아트·오피스북·밤공유·대밤·마사지퀸·퀸알바·인천달리기·부산달리기 등 플랫폼별 SEO 최적화 글. 에이유디자인.",
  keywords: [
    "오피가이드 배너제작",
    "오피가이드 배너 제작",
    "배너 제작 가이드",
...
export default function GuidePage() {
  return (
    <div className={styles.wrapper}>
      <Header />
      <main id="main-content" className={styles.main} aria-label="배너·프로필 제작 가이드">
        <section className={styles.platformPageHero} aria-labelledby="guide-page-title">
          <div className={styles.container}>
            <Breadcrumb items={[{ label: "홈", href: "/" }, { label: "배너 제작 가이드" }]} />
            <h1 id="guide-page-title" className={styles.platformPageLead}>
        ...
        <section className={styles.section} aria-labelledby="guide-list-title">
          <div className={styles.container}>
            <h2 id="guide-list-title" className={styles.sectionTitle}>
              가이드 목록
            </h2>
            <ul className={styles.guideList} aria-label="가이드 글 목록">
              {getOrderedGuidePosts().map((post) => (
                <li key={post.slug} className={styles.guideListItem}>
                  <Link
                    href={`/guide/${post.slug}`}
                    className={styles.guideListLink}
                  >
